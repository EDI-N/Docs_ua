...
Для створення Замовлення (ORDERS), необхідно заповнити ствопці **Кіл-ть одиниць** та **Кіл-ть коробок** враховуючи встановлені обмеження.

.. image:: pics_dlia_dystrybiutora/Instruktsiia_dlia_dystrybiutora_04.png
   :align: center

Після введення замовляємої кількості по позиції, в сервісі EDIN-DISTRIBUTION автоматично для цієї позиції та для всього документа `Замовлення (ORDERS) <https://wiki.edin.ua/uk/latest/Distribution/EDIN_2_0/XML/XML_structure.html#order>`__ розраховуються значення:
