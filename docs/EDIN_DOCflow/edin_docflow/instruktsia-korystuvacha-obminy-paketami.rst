...
------------------------
 ...
 - Тип пакета (комплекта) – налаштування та параметри пакета документів для обміну.
...
**6.3 Обробка документів у пакеті**
...
Для перегляду (ознайомлення) з документом натисніть на його назву або на іконку «Скачати архів» навпроти документу, а для скачування всіх документів у пакеті натисніть «Завантажити архів»:
...
Під *обробкою пакету* слід розуміти надання пакету відповідного статусу. Статус пакету можливо встановити лише за умови, що всі документи в пакеті оброблені. Для встановлення потрібного статусу натисніть відповідну кнопку: «Прийняти», «Відхилити», «Уточнення»:
...
.. admonition:: Зверніть увагу! 
   ...
   Якщо при створенні явилася помилка "Не налаштоване правило узгодження документа" - потрібно налаштувати документ у розділі «Керування доступом -> Узгодження».
...
Для кожного типу документа процес узгодження налаштовується окремо. Для налаштування правил і послідовності процесу узгодження перейдіть до розділу «Керування доступом» - «`Узгодження`_». 
...
Обовязкова умова - увімкнути "Доступ до документу" у пункті `Налаштування доступу для провайдера`_.
...
Використання ієрархічних довідників полегшує узгодження документів в пакеті. 
...
**Налаштування на стороні одержувача пакета**

Налаштування маршруту узгодження проводиться на стороні отримувача пакета. Для цього необхідно:
...
**Налаштування на стороні відправника пакета (контрагент)**

При відпраці пакета, контрагентам більше не потрібно шукати та вказувати відповідних узгоджувачів для різних документів, а просто вибрати потрібні значення при відпраці пакета. Для цього:

1. У пакеті по кожному файлу документа заповнити значення довідників до "фінальних" і відправити пакет.

.. image:: pics_instruktsia_korystuvacha_obminy_paketami/instruktsia_korystuvacha_obminy_paketami_118.png
   :align: center

11 Копіювання пакету с документами, позначеними для редагування
--------------------------------------------------------------------------------------------------------------------

У *Налаштування - Налаштування комплектів документів*, при `додаванні звязку пакета з типами документів <https://wiki.edi-n.com/uk/latest/EDIN_DOCflow/edin_docflow/instruktsia-biznes-administratora.html#pack_with_docs>`_  можливо налаштувати правила роботи з документом.
...
Усі документи з відміткою «Документ дозволений для редагування партнером» буде позначено у пакеті спеціальною іконкою зі спливаючою підказкою, а також додано примітку у блоці *Додаткова інформація*.

.. image:: pics_instruktsia_korystuvacha_obminy_paketami/instruktsia_korystuvacha_obminy_paketami_119.png
   :align: center

Отримувач вхідного пакета з такими документами може створити копію пакету документів за допомогою відповідної кнопки **Створити у відповідь копію**. Новий створений пакет буде збережено у Чернетках.

У новому пакеті у вкладці "Документи" відображаються тільки документи з ознакою «Дозволений для редагування партнером» в статусі "Новий"  - отримувач може редагувати структуровані документи, тобто заповнити їх інформацією. 

До скопійованих типів документів застосовуються усі налаштування правил роботи з документом з пакета-основи. Наприклад необхідна кількість підписів ініціатора і клієнта, допустима кількість файлів та термін підписання. Партнер не зможе редагувати налаштування.
Номер пакета-основи зберігається у вкладці "Основна інформація" при опрацюванні пакету.
...
У вкладках "Довідники" й "Користувацькі поля" зберігаються налаштування пакету-основи. Журнал пакету-основи не зберігається. Неможливо прикріпити довкладення.
