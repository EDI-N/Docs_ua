...
За процесом документообігу «Рахунок (Логістика)» (COMDOC_040) створюється на основі «Підтвердження заявки на транспортування» (TRANSPORTATIONORDER із SUBDOCTYPE = 2).

Якщо під час роботи з «е-ТТН» були створені та підписані Акт перевантаження (ReloadAct), Акт про зміну пункту призначення (ConsigneeChangeAct) або Акт розбіжностей (DisagreementAct), в такому випадку для внесення змін до кінцевого "Рахунку (Логістика)" (COMDOC_040) ПрАТ «Монделіс Україна» формує нову “Заявку на транспортування” (TRANSPORTATIONORDER із SUBDOCTYPE = 1), на підставі якого і створюється "Рахунок (Логістика)" (COMDOC_040).

**Перевізник** для створення в сервісі "EDI Network" **"Рахунку (Логістика)" (COMDOC_040)** :
