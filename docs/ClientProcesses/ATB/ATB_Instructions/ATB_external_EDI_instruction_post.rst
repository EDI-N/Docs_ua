...
.. note::
   При створенні ТТН у стандартному ланцюжку перевезень для АТБ документу присвоюється атрибут **branch_uuid** 

   У випадку створення Акта примусового завершення ТТН необхідно враховувати:

     * акт примусового завершення повністю зупиняє подальшу роботу з ТТН;
     * акт наслідує атрибут branch_uuid з відповідної ТТН;
     * завдяки цьому ТТН та акт залишаються в одному ланцюжку поставки.

   Інші типи актів не зупиняють ТТН, а виконують функцію її коригування відповідно до бізнес-процесу.
...
.. attention::
   З метою зменшення кількості помилок у ТТН, які пов'язані з некоректним GLN компанії та назвами пунктів Навантаження та Розвантаження було створенно наступні перевірки:
    
   1. Перевірка GLN компанії учасника

      Якщо для компанії визначено фіксований список GLN, користувач повинен обрати значення з цього списку. В зворотньому випадку рядок підсвічується червоним кольором. 

   2. Перевірка назв пунктів Навантаження та Розвантаження 
      
      Якщо в блоці Пункт Навантаження та Пункт Розвантаження відсутня назва - рядок підсвічується червоним кольором.

   .. image:: pics_ATB_external_EDI_instruction/ATB_external_EDI_instruction_156.png
      :align: center

   .. image:: pics_ATB_external_EDI_instruction/ATB_external_EDI_instruction_157.png
      :align: center


.. attention::
   Правила роботи з ТТН-документами (тільки для чернеток) за якими виконується контроль найменування та адреси для трьох груп даних:

   1. Дані компанії з документа та GLN

   2. Дані пункту Завантаження та GLN

   3. Дані пункту Розвантаження та GLN
   
   Перевіряється, щоб дані, заповнені у тілі документа (назва компанії, адреса), відповідали даним, що зберігаються для обраного GLN.
   У разі невідповідності або відсутності назви — відповідний рядок підсвічується червоним кольором.


   .. image:: pics_ATB_external_EDI_instruction/ATB_external_EDI_instruction_158.png
      :align: center
...
.. attention::
   ...
   5. Повна інформація про компанію.
      ...
      *Будь-які відхилення в електронних документах — зайва кома, неправильний тип лапок або регістр — призводять до помилок.
      Документи з такими помилками не обробляються, а процес приймання товару блокується.*
...
::
      ...
      Додаткові поля, які обов'язкові для заповненя при роботі з мережою "АТБ":
         ...
         В модальному вікні "Відомості про вантаж", що відкривається після натискання на кнопку "+ Додати":
            ...
            4. "Кількість місць";
            5. "Кількість одиниць";
            6. "Одиниця виміру к-сті одиниць";
            7. "Вид пакування";
            8. "Ціна без ПДВ за одиницю, грн";
            9. "Загальна сума без ПДВ, грн".
...
.. note::
   ...
   * переглянути зміст помилки та виконати рекомендовані дії (див. `Інструкцію для постачальника від мережі "АТБ" <https://wiki.edin.ua/uk/latest/ClientProcesses/ATB/ATB_list.html>`__);
...
Для пошуку **"е-ТТН"**, яка отримала від мережі "АТБ" статус з помилкою обробки, потрібно відкрити папку **«Надіслані»** та в рядку пошуку ввести параметр "Помилка". 
Після цього в журналі документів відображатимуться **"е-ТТН"** із позначкою, що свідчить про помилку обробки.

.. image:: pics_ATB_external_EDI_instruction/ATB_external_EDI_instruction_155.png
      :align: center
      :width: 80%

Текст статусу обробки документа відображається у журналі документів — у вигляді інформаційного повідомлення або під час перегляду товарно-транспортної накладної.
...
.. centered:: Робота з 3PL оператором, який виступає Вантажовідправником
...
.. image:: pics_ATB_external_EDI_instruction/ATB_external_EDI_instruction_154.png
      :align: center
      :width: 80%
...
Після того, як мережа "АТБ" сформувала "Акт приймання товару до ВН" (COMDOC_035), його потрібно затвердити підписанням. Для цього у папці **"Вхідні"** потрібно знайти і відкрити Акт приймання, що потребує підписання (комерційний документ в статусі "Очікує на підписання отримувачем"):
...
"Акт приймання товару до ВН" (COMDOC_035) можна **Підписати**:
