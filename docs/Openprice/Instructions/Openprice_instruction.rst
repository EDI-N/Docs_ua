...
Відкрийте та заповніть файл Excel. Колонки виділенні зеленим кольором - **обов'язкові для заповнення**! Деякі стовпці мають підказки до заповнення - наведіть курсор на назву комірки для відображення підказки: 

.. image:: pics_Openprice_instruction/Openprice_instruction_030.png
   :align: center

В шапці таблиці присутні фільтри, що дозволяють відібрати відображаємі значення.

 .. hint::
   Значення комірок ``Категорія -> Група -> Підгрупа`` обираються послідовно з багаторівневого випадаючого списку:
...
- Країна наявності товару
- Категорія найменувань
- Група
- Підгрупа
- Артикул
- Найменування товару
...
.. hint::
   "Країну наявності товару" можливо обрати зі списку, а заповнення опціонального поля "Область наявності товару" доступне лише за "Країна наявності товару"=Україна:

   .. image:: pics_Openprice_instruction/Openprice_instruction_050.gif
      :align: center

Опубліковані позиції одразу відображаються у всіх **Покупців** та будуть діяти при формуванні "Замовлень". Про публікацію нових товарів відправляється повідомлення на e-mail з основними критеріями товарів та посиланням на прайс-лист компанії.

2.1 Перегляд та редагування Вашого "Прайс-листа"
---------------------------------------------------------

Ваш `"Прайс-лист" (PRICAT) <https://wiki.edin.ua/uk/latest/Distribution/EDIN_2_0/XML/XML_structure.html#pricat>`__ поточного GLN з усіма діючими позиціями знаходиться в розділі **"Мій прайс-лист"**. При переході в розділ відкривається нове вікно прайс-листа з усіма доданими позиціями, згрупованими за наступною ієрархією: ``Категорія -> Група -> Підгрупа`` продукції:
...
У таблиці відображається коротка інформація про Продавця (Назва, ЄДРПОУ, GLN), вказується область чи країна наявності товару, Група товару.

За допомогою зручного рядка `Пошуку <https://wiki.edin.ua/uk/latest/Openprice/Instructions/Openprice_instruction.html#search>`__ (в верхній частині екрану) можливо відфільтрувати потрібних контрагентів за назвою/ЄДРПОУ/GLN компанії, країною/областю доступності товару чи ієрархією товару (``Категорія -> Група -> Підгрупа``):

.. image:: pics_Openprice_instruction/Openprice_instruction_049.gif
   :align: center

.. attention::
   Для пошуку/фільтрації можливо вказати лише один параметр. Для того аби **"Скинути відібрані Прайс-листи"** потрібно натиснути на кнопку |non| в пощуковому рядку.
...
Відкриється нове вікно прайс-листа з усіма доданими позиціями, згрупованими за наступною ієрархією: ``Категорія -> Група -> Підгрупа`` продукції:
...
Пошук документів/позицій в сервісі **Є-Товар** виконується через рядок **"Пошуку"**:

.. image:: pics_Openprice_instruction/Openprice_instruction_051.png
   :align: center

.. hint::
   Загальна концепція `роботи з пошуком <https://wiki.edin.ua/uk/latest/general_2_0/rabota_s_platformoj_EDIN_2.0.html#doc-search>`__ викладена за посиланням.
...
.. note::
   В залежності від типу журналу, який переглядає користувач (відкритий **Прайс-лист**, **Доступні прайс-листи** контрагентів чи папки "документообміну" (**Вхідні**, **Надіслані**, **Чернетки**, **Архів**)) ключі пошуку будуть відрізнятись:
...
.. note::
   В залежності від типу журналу, який переглядає користувач (відкритий **Прайс-лист** чи **Доступні прайс-листи** контрагентів) меню пошуку буде відрізнятись:
