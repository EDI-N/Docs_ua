...
З міркувань безпеки та коректної ідентифікації Постачальника заявка відправляється тільки після накладення електронного цифрового підпису Відправника.

.. hint::
   Процес підписання не відрізняється від підписання будь-якого іншого документа та описаний в `розділі вище <https://wiki.edin.ua/uk/latest/Personal_Cabinet/PCInstruction.html#sign>`__ .

Після підписання формується запит в мережу (в якості чернетки). Перед тим, як **"Відправити запит"** в мережу можливо **"Завантажити PDF заявки"** та додати додаткові файли без підпису (**"+Додати файли"**):
